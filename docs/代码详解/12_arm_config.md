...
- **L8–L13**：四条"关键事实"，是整个文件数值的推导依据：
  ...
  2. **机器人 IP**：官方示例默认 `127.0.0.1`（本机模拟器）；实机要填控制柜网口 IP（如 `192.168.1.200`）；
...
L19  ARM_CONFIG = {
L20      # ---- RPC 连接 ----
L21      "ip": "192.168.1.200",      # 机械臂控制柜 IP（示例默认 127.0.0.1 为模拟器/本机）
...
|---|---|---|---|---|---|
| `ip` | `"192.168.1.200"` | str | 机械臂控制柜的 IP 地址 | `arm_controller.py` L81（构造器默认）、`main_gui_arm.py` L76/L150（GUI 输入框默认值）、`apps/diag_arm.py` L38、`apps/test_arm.py` L61（命令行默认值） | 实机控制柜网口 IP 示例；官方示例默认 `127.0.0.1` 是模拟器/本机。现场若网段不同需在此改 |
