...
class Auboi5Robot:
    ...
    def connect(self, ip='192.168.2.101', port=8899):
        """"
        * FUNCTION:    connect
    ...
    def move_to_target_in_cartesian(self, pos, rpy_xyz):
        ...
        * NOTES:
        ...
        # 打印传入的目标位置和姿态
        logging.info(f"目标位置: {pos}, 姿态: {rpy_xyz}")
        ...
        if self.rshd >= 0 and self.connected:
            ...
            if ik_result is None:
                logging.error("逆运动学解算失败，无法到达目标位置: pos={}, rpy_xyz={}".format(pos, rpy_xyz))
                return RobotErrorType.RobotError_Move  # 避免继续执行

            logging.info(f"逆运动学解算成功，关节角结果: {ik_result['joint']}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 连接机械臂
        ip = '192.168.1.100'  # 或者你的机械臂 IP
        port = 8899
        result = robot.connect(ip, port)
        if result != RobotErrorType.RobotError_SUCC:
            raise Exception(f"连接服务器 {ip}:{port} 失败")
        ...
        # 获取当前机械臂的关节状态
        current_joint_angles = robot.get_current_waypoint()['joint']
        print(f"当前关节角度: {current_joint_angles}")

        # 获取正向运动学结果
        fk_result = robot.forward_kin(current_joint_angles)
        if fk_result is None:
            raise Exception("正向运动学计算失败")
        

        # 当前关节角度: [-3.28572416305542, -0.2305155247449875, -1.193629503250122, 0.49577799439430237, -1.5568468570709229, -0.2396542876958847]
        # 位置: [0.34206193635060694, 0.07445181509093249, 0.6284345080601109], 
        # 姿态: [0.04246358552659462, 0.6711834039317383, 0.7391462074123769, 0.03704816134074899]


        # 提取旋转矩阵和平移向量
        position = fk_result['pos']
        orientation = fk_result['ori']
        print(f"位置: {position}, 姿态: {orientation}")
