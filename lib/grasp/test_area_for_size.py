...
from newadaptive_grasp import adaptive_open_apple, adaptive_close_apple
...
limitspeed = 0.1          # tree fruit grasp: slow and stable
...
# 放水果到右边纸箱
high_hover_pos_shuiguo = (-0.47, -0.385113 + offsety , 0.52)
...
#   苹果  梨子  香蕉  橘子  芒果
action_release = [1]   # apple open hand action
action_grab    = [0]   # apple grasp action
...
async def grasp_and_place_object_task10(
    ...
    global grasp_objects, target_objects, take_order, _display_stop
    ...
    processed_objects = []
    take_objs  = take_order[:4]   # 取全部5种水果
    num2       = len(take_objs)   # num2=4，要抓4个
    take_order1 = take_objs.copy()
    start_time  = time.time()

    # 开始录制全程（新增）
    if SAVE_VIDEO:
        os.makedirs(VIDEO_DIR, exist_ok=True)
        fname  = f"{VIDEO_DIR}/grasp_all_{int(time.time())}.mp4"
    ...
    while take_objs:
        ...
        if results[0].masks is not None:
            ...
            for obj in cls:
                ...
                # X_cam = (xs_valid-cam_intrinsics["cx"]) * depths_valid / cam_intrinsics["fx"]
                # Y_cam = (ys_valid-cam_intrinsics["cy"]) * depths_valid / cam_intrinsics["fy"]
                # Z_cam = depths_valid
                #
                # P_cam = np.array(
                #     [
                #         float(np.median(X_cam)),
                #         float(np.median(Y_cam)),
                #         float(np.median(Z_cam))
                #     ]
                # )
                # depth_value = float(np.median(depths_valid))
                # area = np.sum(valid) * (depth_value/cam_intrinsics["fx"]) * (depth_value/cam_intrinsics["fy"])
                # area = area*1e4
                #
                # center_pixel = (
                #     int(np.median(xs_valid)),
                #     int(np.median(ys_valid)),
                # )
                # print("*"*20)
                # print(depth_value)
                # print(P_cam)
            ...
            num5 = 0
            print( "num3:", num3 , "len_take_order1:", len(take_order1) )
            # 准备抓取目标对象
            take_obj = take_order1[num3]
            print("take_obj:", take_obj)
            num6=1  
            # 先走到要抓取的物品的位置正上方重定位
            #预接近标志位num6
            if num6 == 0:
                ...
                continue
            if num3 < num2 and take_obj in all_objects:
                ...
                print(f"当前移到可乐正上方末端位置: {np.round(end_effector_position, 2)}, 姿态: {np.round(end_effector_quaternion, 2)}")
		    ...
		    # 步骤6：回高悬停位（过渡点）
                    print("[步骤6] 回高悬停位...")
                    retract_pos[1]=retract_pos[1]+0.15
                    ...
                    # 步骤8：降低高度在纸箱放置
                    target_pos = (high_hover_pos_shuiguo[0], high_hover_pos_shuiguo[1], 0.40 )
